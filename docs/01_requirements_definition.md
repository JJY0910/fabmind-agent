...
FabMind Agent의 목표는 반도체 Load Port / FOUP Clamp / EtherCAT I/O 장비군에서 발생하는 대표 장애를 현장 엔지니어가 빠르게 분석하고 보고할 수 있도록 돕는 것이다. 제품은 보안과 안전을 위해 읽기 전용 진단·교육·보고 시스템으로 설계한다.

## 2. 성공 기준

운영 품질 기준을 만족하기 위한 최소 성공 기준은 다음과 같다.

1. 하나의 Golden Path가 끊기지 않고 동작한다.
2. AI 분석 결과는 근거와 함께 표시된다.
3. 위험 조치는 자동 권고하지 않고 승인 플로우를 요구한다.
4. LLM 없이도 deterministic rule engine으로 재현 가능한 분석 결과를 제공한다.
5. GitHub CI에서 backend, frontend, E2E 테스트가 실행된다.
6. README와 제품 문서만 봐도 문제-제약-해결-운영 경계가 이해된다.
...
| Reviewer | 운영 품질/시스템 검토자 | synthetic data 기반 읽기 전용 검토 |
...
### FR-10 Operational Scenario Runner

- 운영 흐름 검증을 위해 20개 이상의 seed scenario를 제공한다.
